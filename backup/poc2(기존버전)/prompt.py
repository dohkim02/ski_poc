...
이 데이터들을 바탕으로 '지속적이고 복구되지 않는 하락 추세(내리막 패턴)'에 들어섰는지를 판단해줘.

단순히 평균보다 낮거나 일시적인 등락이 있는 패턴은 제외하고,  
'한 번 하락한 이후 반등 없이 지속적으로 하락하거나 낮은 수준이 유지되는 경우'만  
이상 패턴으로 판단해줘.

즉, 다음은 이상이 **아닌 경우**로 간주한다:
- 수치가 일정하게 유지되는 경우
- 수치가 오르락내리락 반복하는 경우
- 일시적으로 하락한 후 반등한 경우

다음은 **이상으로 간주해야 할 경우**다:
- 특정 시점부터 계속 하락하거나 낮은 상태로 유지되는 경우
- 하락 후 반등 없이 연속 저조한 수치를 기록하는 경우

과거 연도들과 비교해 전체적으로 데이터가 위 조건에 해당하는지 판단하고,  이상 여부를 판단해줘.

위 패턴을 기반으로 '지속된 내리막' 패턴인지 분석해줘.
단, 25년 데이터가 정상궤도로 돌아오면 이상치가 아니다.

[출력형식]
★이상 여부를 `"yes"` 또는 `"no"`로 답해줘.
★만약 `"yes"`라면, 반드시 다음 정보를 `reason`에 포함해:
- 어떤 달부터 이상이 시작되었는지
- 이전 수치와 비교했을 때 얼마나 낮아졌는지 (언제부터 인지 정확한 수치를 말해줘)
"""
